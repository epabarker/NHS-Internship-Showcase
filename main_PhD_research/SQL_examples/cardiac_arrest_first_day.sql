...
INNER JOIN mimic_derived.first_day_lab ON icustay_detail.stay_id = first_day_lab.stay_id
INNER JOIN mimic_derived.first_day_vitalsign ON icustay_detail.stay_id = first_day_vitalsign.stay_id
ORDER BY subject_id, stay_id), troponin AS
(SELECT
hadm_id,
MAX(troponin_t) AS troponin_t_max
FROM mimic_derived.cardiac_marker
GROUP BY hadm_id)
