...
-- (b) a diagnosis of acute organ dysfunction (Appendix 2).
 ...
 WITH dx AS
     (
       ...
       FROM postgres.mimic_hosp.diagnoses_icd
     )
     , dx_icd9 AS
     (
        SELECT hadm_id,
     ...
     (
        ...
        FROM postgres.mimic_hosp.procedures_icd
         WHERE icd_version = 9
         GROUP BY hadm_id
     )
     , proc_icd10 AS
     ...
     , aggregate as
     (
        SELECT mimic_core.admissions.subject_id, mimic_core.admissions.hadm_id
        ...
        FROM postgres.mimic_core.admissions
         LEFT JOIN dx_icd9
             ON mimic_core.admissions.hadm_id = dx_icd9.hadm_id
         LEFT JOIN dx_icd10
             ON mimic_core.admissions.hadm_id = dx_icd10.hadm_id
         LEFT JOIN proc_icd9
             ON mimic_core.admissions.hadm_id = proc_icd9.hadm_id
         LEFT JOIN proc_icd10
             ON mimic_core.admissions.hadm_id = proc_icd10.hadm_id
     )
